...
class TestLAD(unittest.TestCase):
    ...
    def test_LAD_load(self):
        ...
        lad_from_shp = LAD.from_shapefile(lad_hr_pth, area_var='Area', name='CIR', region_var='Region4', other_vars=['Category4'])
        lad_from_shp = LAD.from_shapefile(lad_lr_pth, area_var='Lake_area', idx_var='Hylak_id', name='HL', region_var=None)
        lad_from_shp = LAD.from_shapefile(lad_lr_pth, area_var='Lake_area')
        lad_from_shp = LAD.from_shapefile(lad_hr_pth, computeArea=True, areaConversionFactor=1e6)
    ...
    def test_LAD_load_from_gdf(self):
        ...
        gdf = pyogrio.read_dataframe(lad_hr_pth, read_geometry=True, use_arrow=True)
    ...
    def test_concat(self):
        ...
        lad_concat = LAD.concat((lad_from_shp, lad_from_shp), broadcast_name=True)
    ...
    def test_plots(self):
        lad_from_shp = LAD.from_shapefile(lad_lr_pth, area_var='Lake_area')
        plotEPDFByValue(lad_from_shp.Area_km2) # Should produce a plot in debug mode
    ...
    def test_workflow(self):
        ...
        print(f"Mean LEV: {lad_lev.sumLev(asFraction=True, includeExtrap=False)['mean']:.2%}")
